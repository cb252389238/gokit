...
// Parse 智能解析出省市区+街道地址
func Parse(address *Address, str string) *Address {
	// 匹配所有省级
	...
	if len(pArr) == 0 {
		fmt.Println(111)
		rArr = strings.Split(str, " ")
		fmt.Println(rArr)
		for _, vprovince := range rArr {
			provinceReg := regexp.MustCompile(vprovince)
			for k1, v1 := range areaMap.ProvinceByName {
				provinces := provinceReg.FindAllString(k1, -1)
				if len(provinces) > 0 {
					address.Province = v1[0].Name
				}
			}

			cityReg := regexp.MustCompile(vprovince)
			for k1, v1 := range areaMap.CityByName {
				citys := cityReg.FindAllString(k1, -1)
				if len(citys) > 0 {
					address.City = v1[0].Name
				}
			}

			regionReg := regexp.MustCompile(vprovince)
			for k1, v1 := range areaMap.RegionByName {
				regions := regionReg.FindAllString(k1, -1)
				if len(regions) > 0 {
					address.Region = v1[0].Name
				}
			}
		}
		return address
