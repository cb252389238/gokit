...
func Load(path string) {
	...
	initialPath = path
	configFullName, configSuffix, configPrefix = easy.FileInfo(path)
	configPath = easy.GetRootPath() + strings.Replace(path[:len(path)-len(configFullName)], "./", "", -1)
...
func Listen(sec time.Duration) {
	...
	for _ = range ticker.C {
		...
		if err != nil {
			log.Printf("listenConfig is open oriConfig file err:%+v|path:%s", err, configFile)
		} else {
			fileInfo, err := f.Stat()
			if err != nil {
			...
			if err = f.Close(); err != nil {
				log.Printf("oriConfig file close fail error:%+v", err)
			}
		}
	}
}
