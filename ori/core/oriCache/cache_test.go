...
func testFillAndSerialize(t *testing.T, tc *Cache) {
	...
	err := tc.Save(fp)
	if err != nil {
		t.Fatal("Couldn't save oriCache to fp:", err)
	}

	oc := New(DefaultExpiration, 0)
	err = oc.Load(fp)
	if err != nil {
		t.Fatal("Couldn't load oriCache from fp:", err)
	}
...
func TestFileSerialization(t *testing.T) {
	...
	f, err := ioutil.TempFile("", "go-oriCache-oriCache.dat")
	if err != nil {
		t.Fatal("Couldn't create oriCache file:", err)
	}
	fname := f.Name()
	f.Close()
	tc.SaveFile(fname)
