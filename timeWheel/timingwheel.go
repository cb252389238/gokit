...
type (
	...
	TimingWheel struct {
		...
		drainChannel  chan func(key string, value interface{})
...
func newTimingWheelWithClock(interval time.Duration, numSlots int, execute Execute, ticker timex.Ticker) (
	*TimingWheel, error) {
	...
	tw := &TimingWheel{
		...
		drainChannel:  make(chan func(key string, value interface{})),
...
func (tw *TimingWheel) Drain(fn func(key string, value interface{})) {
	tw.drainChannel <- fn
}

// MoveTimer moves the task with the given key to the given delay.
func (tw *TimingWheel) MoveTimer(key string, delay time.Duration) {
	if delay <= 0 || key == "" {
		return
	}
...
func (tw *TimingWheel) RemoveTimer(key string) {
	if key == "" {
		return
	}
...
func (tw *TimingWheel) SetTimer(value interface{}, delay time.Duration) string {
	key := tw.snowflake.Generate().String()
	if delay <= 0 || key == "" {
		return ""
	}
...
func (tw *TimingWheel) drainAll(fn func(key string, value interface{})) {
