...
func (this *LocalLogger) SetLogger(adapterName string, configs ...string) error {
	...
	for i, l = range this.outputs {
		if l.name == adapterName {
			if l.config == config {
				//配置没有变动，不重新设置
				return fmt.Errorf("you have set same coreConfig for this adaptername %s", adapterName)
			}
			l.Logger.Destroy()
			num = i
			break
...
func (this *LocalLogger) writeToLoggers(when time.Time, msg *loginfo, level int) {
	for _, l := range this.outputs {
		...
		if this.format != "json" {
			...
			var marshal, _ = json.Marshal(msg)
			msgStr = string(marshal)
		}
