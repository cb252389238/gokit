...
// new file name like  xx.2013-01-01.001.logs
func (f *fileLogger) createFreshFile(logTime time.Time) error {
	...
	if err == nil {
		return fmt.Errorf("Cannot find free logs number to rename %s", f.Filename)
	}
	f.fileWriter.Close()
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "os.Rename %s to %s err:%+v\n", f.Filename, fName, err)
		goto RESTART_LOGGER
	}
...
func (f *fileLogger) deleteOldLog() {
	...
	filepath.Walk(dir, func(path string, info os.FileInfo, err error) (returnErr error) {
		defer func() {
			if r := recover(); r != nil {
				fmt.Fprintf(os.Stderr, "Unable to delete old logs '%s', error: %v\n", path, r)
			}
		}()
