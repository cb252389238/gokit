...
var (
	logOnce      sync.Once
	LogSingleObj *logger.LocalLogger
)

func NewLog() *logger.LocalLogger {
	logOnce.Do(func() {
		allConfig := GetAllHotConf()
		path := common.GetRootPath()
		dir := path + "log"
		_, err := common.MakeDir(dir)
		if err != nil {
			panic(fmt.Sprintf("创建日志目录失败 path:%s", dir))
		}
		filename := strings.Replace(dir+"/metaServer.log", "\\", "/", -1)
		var level string
		if allConfig.Debug {
			//debug 模式
			level = "TRAC"
    ...
    "File": {
		...
		err = logger.SetLogger(loggerJson)
		if err != nil {
			panic(err)
		}
		LogSingleObj = logger.New()
