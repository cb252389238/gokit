...
func Run(ctx *svc.ServiceContext) {
	...
	go func() {
		if err := server.Serve(listener); err != nil {
			resource.LogInfo("http service退出,err:%+v", err)
	...
	for {
		...
		case sig := <-ctx.HttpC:
			...
			case types.SIGUSR1: //优雅退出
				...
				resource.LogInfo("http service 优雅退出")
				wg.Wait() //登陆用户连接全部断开
				ctx.C <- syscall.SIGINT
			case types.SIGUSR2: //平滑重启
				...
				resource.LogInfo("http service 热重启")
				if err := server.Shutdown(ctx2); err != nil {
					resource.LogError(err.Error())
					break
				}
